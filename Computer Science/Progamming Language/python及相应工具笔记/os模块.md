...
将多个传入路径组合为一个路径。实际上是将传入的几个字符串用系统的分隔符连接起来，组合成一个新的字符串，所以一般的用法是将第一个参数作为父目录，之后每一个参数即使下一级目录，从而组合成一个新的符合逻辑的路径。
...
### **3.8os.path.isfile() 和 os.path.isdir()**
...
将对应路径的文件名和后缀名分割,；默认返回(fname,fextension)元组，可做分片操作
